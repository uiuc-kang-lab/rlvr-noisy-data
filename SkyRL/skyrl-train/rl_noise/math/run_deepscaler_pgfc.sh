...
# PGFC (Cai et al. 2025, arXiv:2510.00915): forward correction applied to the
# score-function terms (w_1=rho_1 for reward 1, w_0=rho_1-1 for reward 0), not by
# shifting rewards in env.py (which GRPO's group-mean subtraction cancels).
# See rl_noise/math/main_math_hard_pgfc.py for why the reward-space version was a no-op.
#
# The FN rate rho_1 is passed as --pgfc_noise_rate=<p>.
# export WANDB_API_KEY=<your_key_here>

NUM_GPUS=4
MODEL_NAME=Qwen/Qwen2.5-Math-7B
RUN_NAME=deepscaler_pgfc
...
uv run --isolated --extra vllm -m rl_noise.math.main_math_hard_pgfc \
  ...
  trainer.algorithm.advantage_estimator="pgfc" \
  trainer.algorithm.pgfc_fn_rate=$PGFC_NOISE_RATE \
