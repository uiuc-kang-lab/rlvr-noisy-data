...
class PolicyWorkerBase(Worker):
    ...
    def ppo_train(self, train_data: TrainingInputBatch) -> TrainingOutputBatch:
        ...
        for epoch in range(self.cfg.trainer.update_epochs_per_batch):
            ...
            for local_step, experience in enumerate(pbar):
                ...
                if "grad_cos_sim" in status:
                    short_status["gd_cos"] = status["grad_cos_sim"]
                    short_status["gd_dn"] = status["grad_diff_norm"]
                    short_status["gd_nA"] = status["grad_norm_A"]
                    short_status["gd_nB"] = status["grad_norm_B"]
    ...
    def gradient_analysis(self, sequences, num_actions, attention_mask, old_action_log_probs, rollout_action_logprobs, base_action_log_probs, loss_mask, accumulation_steps, noisy_advantages, clean_advantages):
        saved_grads = {}
        for name, p in self.model.named_parameters():
            if p.grad is not None:
                saved_grads[name] = p.grad.detach().cpu().clone()
                p.grad = None
        
        def compute_gradients_for_analysis(target_advantages):
            """
            Runs a dedicated forward/backward pass to extract gradients 
            without affecting the main optimizer state.
            """
            # Ensure we start with zero grads for this analysis
            self.optimizer.zero_grad()
            
            with torch.autocast(dtype=torch.bfloat16, device_type="cuda"):
                # Re-run Forward Pass
                # Note: If your model has heavy dropout, gradients might vary slightly 
                # between runs due to RNG. For strict analysis, fix the seed here.
                ...
                total_loss = policy_loss / accumulation_steps

            # Run Backward
            self.strategy.backward(total_loss, self.model, self.optimizer)

            # Flatten and Capture Gradients
            grads = []
            for p in self.model.parameters():
                if p.grad is not None:
                    ...
                    # Check if it is a DTensor (Distributed Tensor)
                    # If so, extract the local shard as a plain Tensor.
                    # This prevents torch.cat from triggering a distributed all_gather.
                    if hasattr(g, "to_local"):
                        g = g.to_local()
                    
                    # Now it is safe to move to CPU and flatten
                    grads.append(g.cpu().flatten())
        ...
        grad_noisy = compute_gradients_for_analysis(noisy_advantages)
        grad_clean = compute_gradients_for_analysis(clean_advantages)
        
        cos_sim = F.cosine_similarity(grad_noisy.unsqueeze(0), grad_clean.unsqueeze(0)).item()
        diff_norm = torch.norm(grad_noisy - grad_clean).item()
        norm_A = torch.norm(grad_noisy).item()
        norm_B = torch.norm(grad_clean).item()

        for name, p in self.model.named_parameters():
            if name in saved_grads:
                p.grad = saved_grads[name].to(p.device)
                
        return cos_sim, diff_norm, norm_A, norm_B
    ...
    def training_step(self, experience: Experience, global_step, local_step, accumulation_steps) -> Dict[str, float]:
        ...
        additional_advantages = experience.additional_advantages
        num_actions = experience.num_actions
        attention_mask = experience.attention_mask
        loss_mask = experience.loss_mask
        rollout_action_logprobs = experience.rollout_logprobs
        
        if additional_advantages is not None:
            cos_sim, diff_norm, norm_A, norm_B = self.gradient_analysis(
                sequences,
                num_actions,
                attention_mask,
                old_action_log_probs,
        ...
        if additional_advantages is not None:
            status["grad_cos_sim"] = cos_sim
            status["grad_diff_norm"] = diff_norm
            status["grad_norm_A"] = norm_A
            status["grad_norm_B"] = norm_B
        return status
