...
def convert_prompts_responses_to_batch_tensors(
    ...
    additional_rewards: Optional[List[List[Optional[float]]]] = None,
) -> Tuple[
    ...
    ret_additional_rewards = None
    if additional_rewards:
        ret_additional_rewards = torch.zeros_like(action_mask, dtype=torch.float)
        for i, additional_reward in enumerate(additional_rewards):
            if isinstance(additional_reward, list):
                additional_reward = torch.tensor(additional_reward)
            ret_additional_rewards[i, : len(additional_reward)] = additional_reward
    ...
    return sequences, attention_mask, action_mask, ret_custom_rewards, ret_loss_masks, ret_additional_rewards, logprobs_tensor
