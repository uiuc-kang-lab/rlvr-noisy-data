...
class RayPPOTrainer:
    ...
    async def eval(self) -> Dict[str, float]:
        ...
        if self.cfg.trainer.dump_eval_results:
            with Timer("dump_eval_results"):
                data_save_dir = (
                    Path(self.cfg.trainer.export_path) / "dumped_evals" / f"global_step_{self.global_step}_pass@{self.cfg.generator.eval_n_samples_per_prompt}_temp{self.cfg.generator.eval_sampling_params.temperature}_evals"
                )
    ...
    def train(self):
        ...
        if self.cfg.trainer.save_only:
            logger.info("Save only mode, saving model and exiting...")
            self.save_models()
            return
    ...
    def build_models(self, PolicyWorker, CriticWorker, RefWorker, RewardWorker=None):
        ...
        if RewardWorker is not None and cfg.trainer.reward.model.path:
            raise NotImplementedError("reward models arxe not supported yet")
    ...
    def convert_to_training_input(self, generator_output: GeneratorOutput, uids: List[str]) -> TrainingInputBatch:
        ...
        additional_rewards: List[List[Optional[float]]] = generator_output.get("additional_rewards", None)
        ...
        training_input = TrainingInputBatch(
            {
                ...
                "additional_rewards": additional_rewards_tensor,
    ...
    def postprocess_generator_output(self, generator_output: GeneratorOutput, uids: List[str]) -> GeneratorOutput:
        ...
        additional_rewards = generator_output.get("additional_rewards", None)
        responses: List[List[int]] = generator_output["response_ids"]
        per_token_rewards: List[List[float]] = []
        per_token_additional_rewards: List[List[Optional[float]]] = []
        ...
        if additional_rewards and isinstance(additional_rewards[0], list):
            per_token_additional_rewards = additional_rewards
        elif additional_rewards and additional_rewards[0] is not None:
            for additional_reward, response in zip(additional_rewards, responses):
                per_token_add_reward = [0.0] * len(response)
                per_token_add_reward[-1] = float(additional_reward)
                per_token_additional_rewards.append(per_token_add_reward)
        ...
        if additional_rewards:
            generator_output["additional_rewards"] = per_token_additional_rewards
        return generator_output
    ...
    def compute_advantages_and_returns(self, data: TrainingInputBatch) -> TrainingInputBatch:
        ...
        if data["additional_rewards"] is not None:
            additional_advantages, additional_returns = ppo_utils.compute_advantages_and_returns(
                token_level_rewards=data["additional_rewards"],
                response_mask=data["response_mask"],
                index=data.metadata["uids"],
                adv_estimator=self.cfg.trainer.algorithm.advantage_estimator,
                config=self.cfg.trainer.algorithm,
            ...
            data["additional_returns"] = additional_returns
            data["additional_advantages"] = additional_advantages
        else:
            data["additional_returns"] = None
            data["additional_advantages"] = None
