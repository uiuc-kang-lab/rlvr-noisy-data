...
class SkyRLGymGenerator(GeneratorInterface):
    ...
    async def generate_batched(
        ...
        for i, (response, response_ids, env) in enumerate(zip(responses, all_response_ids, envs)):
            ...
            reward = env_step_output["reward"]
            additional_reward = env_step_output["additional_reward"] if "additional_reward" in env_step_output else None
            extra_metadata.append(env_step_output.get("metadata", {}))
            rewards.append(reward)
            additional_rewards.append(additional_reward)
        ...
        generator_output: GeneratorOutput = {
            ...
            "rewards": rewards,
            "additional_rewards": additional_rewards if additional_rewards[0] is not None else None,
