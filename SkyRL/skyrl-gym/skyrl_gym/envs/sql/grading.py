...
def safe_execute_sql(db_path, query, db_modification_str=None):
    conn = sqlite3.connect(db_path, autocommit=False)
    cursor = conn.cursor()
    try:
        if db_modification_str:
            cursor.executescript(db_modification_str)
        cursor.execute(query)
        result = cursor.fetchall()
        result = remove_null_rows(result)
        conn.rollback()  # Rollback any modifications
        return result, None
